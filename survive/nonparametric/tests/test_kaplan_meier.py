...
def test_no_censoring_one_group():
    ...
    for conf_type, var_type, tie_break in product(*KM_PARAMETERS):
        ...
        assert kaplan_meier.data_.n_groups == 1
        ...
        assert isinstance(y_pred, pd.DataFrame)
        assert isinstance(y_se, pd.DataFrame)
        np.testing.assert_almost_equal(y_pred, y_true)
        np.testing.assert_equal(y_pred.shape,
                                (len(x), kaplan_meier.data_.n_groups))
        ...
        assert isinstance(y_pred, pd.DataFrame)
        assert isinstance(y_lower, pd.DataFrame)
        assert isinstance(y_upper, pd.DataFrame)
        np.testing.assert_almost_equal(y_pred, y_true)
        np.testing.assert_equal(y_pred.shape,
                                (len(x), kaplan_meier.data_.n_groups))
        ...
        assert isinstance(q_pred, pd.DataFrame)
        np.testing.assert_almost_equal(q_pred, q_true)


def test_censoring_one_group():
    ...
    for conf_type, var_type, tie_break in product(*KM_PARAMETERS):
        ...
        assert isinstance(y_pred3, pd.DataFrame)
        assert isinstance(y_lower, pd.DataFrame)
        assert isinstance(y_upper, pd.DataFrame)
        ...
        y_pred4, y_se, y_lower, y_upper = \
            kaplan_meier.predict(x, return_se=True, return_ci=True)
        assert isinstance(y_pred4, pd.DataFrame)
        assert isinstance(y_se, pd.DataFrame)
...
def test_no_censoring_two_groups():
    ...
    for conf_type, var_type, tie_break in product(*KM_PARAMETERS):
        ...
        assert kaplan_meier.data_.n_groups == 2
...
def test_leukemia():
    ...
    survival_control = [1., 19 / 21, 17 / 21, 16 / 21, 14 / 21, 12 / 21, 8 / 21,
                        6 / 21, 4 / 21, 3 / 21, 2 / 21, 1 / 21, 0.]
    ...
    ci_lower_log_log = [0.6197180, 0.5631466, 0.5031995, 0.4316102, 0.3675109,
                        0.2677789, 0.1880520]
    ci_upper_log_log = [0.9515517, 0.9228090, 0.8893618, 0.8490660, 0.8049122,
                        0.7467907, 0.6801426]
    ...
    ci_lower_linear = [0.7074793, 0.6363327, 0.5640993, 0.4808431, 0.4039095,
                       0.2864816, 0.1843849]
    ci_upper_linear = [1.0000000, 0.9771127, 0.9417830, 0.8995491, 0.8509924,
                       0.7891487, 0.7119737]
